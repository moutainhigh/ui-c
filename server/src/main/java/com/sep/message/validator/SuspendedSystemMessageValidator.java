...
import com.sep.message.dto.SuspendedSystemMessageDto;
...
@Component
public class SuspendedSystemMessageValidator {

    @Resource
    private SystemMessageService systemMessageService;

    public void validator(SuspendedSystemMessageDto dto) {
        LambdaQueryWrapper<SystemMessage> lambda = new QueryWrapper<SystemMessage>().lambda();
        lambda.in(SystemMessage::getId, dto.getIds());
        lambda.ne(SystemMessage::getStatus, SystemMessageStatus.PUBLISHED.getCode());
        int count = systemMessageService.count(lambda);
        if (count > 0) {
            throw new SepCustomException(BizErrorCode.SYSTEM_MESSAGE_SUSPENDED_STATUS_ERROR);
        }
    }
