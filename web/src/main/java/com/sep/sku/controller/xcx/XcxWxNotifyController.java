 ...
 */
@RestController
@RequestMapping("/xcx/wx")
@Api(value = "微信支付回调", tags = {"微信支付回调相关API"})
@Slf4j
public class XcxWxNotifyController extends BaseController {
    ...
    private String validateWxNotify(Map map) {
        ...
        if (sign.equals(map.get("sign"))) {
            ...
            if ("A".equals(orderNo.substring(0,1))) {
                return active(orderNo,totalPrice,wxTradeNo);
            } else {
                return order(orderNo,totalPrice,wxTradeNo);
            }
        } else {
    ...
    private String order(String orderNo,Integer totalPrice,String wxTradeNo) {
        log.info(orderNo);
        Order order = orderService.getSkuOrderInfoByOrderNo(orderNo);
        log.info("【wx_notify】订单信息：{}", JSON.toJSONString(order));

        if (order == null) {
            log.warn("【wx_notify】订单不存在，order_no：{}", orderNo);
            // 没有订单，返回成功
            return wxReturnOkInfo();
        }
        if (order.getStatus() == OrderStatus.OVER.getCode()) {
            log.warn("【wx_notify】订单已处理，order_no：{}", orderNo);
            // 订单已处理，返回成功
            return wxReturnOkInfo();
        }
        if (NumberArithmeticUtils.safeMultiply(order.getAmount(), 100).intValue() != totalPrice) {
            log.error("【wx_notify】订单金额不正确，order_no：{},total_Price:{}", orderNo, totalPrice);
            // 订单金额不正确，返回失败
            return wxReturnFailInfo("订单金额不正确");
        }
        // 修改订单、订单sku及交易日志表
        WxNotifyDealDto wxNotifyDealDto = new WxNotifyDealDto();
        wxNotifyDealDto.setOrderNo(orderNo);
        wxNotifyDealDto.setWxTradeNo(wxTradeNo);
        try {
            orderService.wxNotifyDeal(wxNotifyDealDto);
        } catch (Exception e) {
            e.printStackTrace();
            return wxReturnFailInfo("商户订单处理失败");
        }
        return wxReturnOkInfo();

    }

    private String active(String orderNo,Integer totalPrice,String wxTradeNo) {
